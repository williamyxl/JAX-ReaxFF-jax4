...
def replace_pos_with_subs(minim_index_lists, list_all_pos, list_all_pos_sub):
    ...
    for i in range(len(list_all_pos)):
        if len(minim_index_lists[i]) != 0:
            if isinstance(list_all_pos[i], np.ndarray):
                list_all_pos[i].at[minim_index_lists[i]].set(list_all_pos_sub[ctr])
            else:
                list_all_pos[i][minim_index_lists[i]] = list_all_pos_sub[ctr]
...
def use_selected_parameters(params,param_indices, flattened_force_field):
    for i, ind in enumerate(param_indices):
        if isinstance(flattened_force_field[ind[0]], np.ndarray):
                flattened_force_field[ind[0]].at[ind[1]].set(params[i])
        else:
            flattened_force_field[ind[0]][ind[1]] = params[i]
...
def loss(flattened_force_field,flattened_non_dif_params,
    ...
    for i in range(len(list_all_type)):
        ...
        if charge_items_flag:
            if isinstance(all_charges, np.ndarray):
                all_charges.at[jax.ops.index[start:end, :atom_counts[i]]].set(charges)
            else:
                all_charges[jax.ops.index[start:end, :atom_counts[i]]] = charges
            # all_charges = jax.ops.index_update(all_charges, jax.ops.index[start:end, :atom_counts[i]], charges)
        if geo_items_flag:
            if isinstance(all_charges, np.ndarray):
                all_positions.at[jax.ops.index[start:end, :atom_counts[i], :]].set(list_all_positions[i])
            else:
                all_positions[jax.ops.index[start:end, :atom_counts[i], :]] = list_all_positions[i]
        ...
        if force_items_flag:
            ...
            if isinstance(all_charges, np.ndarray):
                all_forces.at[jax.ops.index[start:end, :atom_counts[i], :]].set(forces)
            else:
                all_forces[jax.ops.index[start:end, :atom_counts[i], :]] = forces
